...
const KpiTile: React.FC<{ label: string; value: string | number; sub?: string; tone?: "default" | "accent" }> = ({ label, value, sub, tone = "default" }) => (
  <GlassCard padding={18} style={{ minHeight: 110 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <GlassCard padding={28}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 4 }}>
              <Button tone="primary" onClick={() => navigate("/workspace/projects/new")}>New project</Button>
              <Button tone="outline" onClick={() => navigate("/workspace/teams/new")}>New team</Button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 16 }}>
        {loading ? (
          ...
          <>
            <KpiTile label="Projects" value={overview?.projects?.total ?? projects.length} sub={`${projects.filter(p => p.status === "active").length} active`} />
            <KpiTile label="Teams" value={overview?.teams?.total ?? teams.length} sub={`${teams.filter(t => !t.is_archived).length} live`} />
            <KpiTile label="Tasks completed" value={`${completionPct}%`} sub={`${overview?.tasks?.completed ?? 0} / ${overview?.tasks?.total ?? 0} done`} tone="accent" />
            <KpiTile label="Agent runs (30d)" value={(overview?.agent_runs?.total ?? 0).toLocaleString()} sub={`${overview?.documents?.total ?? 0} docs indexed`} />
          </>
        )}
      </div>
            ...
            <GlassCard padding={6}>
              {leaderboard.length === 0 ? (
                ...
                <div key={idx} style={{
                  ...
                  <AgentChip agentKey={(row.agent_key || row.custom_agent_id || "agent") as string} size={22} />
                ...
                <div key={a.id || idx} style={{
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: tokens.SLATE_500 }}>
                      {a.related_resource_type ? `${a.related_resource_type} ${String(a.related_resource_id || "").slice(0, 6)}…` : "—"}
                    </div>
                  </div>
